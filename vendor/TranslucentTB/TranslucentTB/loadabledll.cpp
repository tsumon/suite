...
std::filesystem::path LoadableDll::GetDllPath(const std::optional<std::filesystem::path> &storageFolder, bool copyDll, std::wstring_view dll)
{
	std::error_code err;
...
LoadableDll::LoadableDll(const std::optional<std::filesystem::path> &storagePath, bool copyDll, std::wstring_view dll) :
	m_hMod(LoadDll(GetDllPath(storagePath, copyDll, dll)))
{
}
